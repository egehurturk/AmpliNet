...
class VAE:
    '''
    Deep convolutional variational autoencoder architecture with 
    mirrored encoder and decoder components
    '''

    def __init__(self, input_shape, conv_filters, conv_kernels, conv_strides, code_size):
    ...
    def compile(self, lr=0.0001):
        ...
        self.model.compile(optimizer = optimizer, loss = self._calculate_loss)
    ...
    def _calculate_loss(self, y_target, y_predicted):
        # calculate reconstruction loss
        error = y_target - y_predicted
        reconstruction_loss = keras.backend.mean(keras.backend.square(error), axis=[1,2,3])

        # calculate Kullback–Leibler divergence loss
        kl_loss = -0.5 * keras.backend.sum(1 + self.log_variance - 
                                           keras.backend.square(self.mu) - 
                                           keras.backend.exp(self.log_variance), axis=1)

        # calculate combined loss (LOSS = α*RMSE + KL)
        combined_loss = self.loss_weight * reconstruction_loss + kl_loss
    ...
    def _build_autoencoder(self):
        ...
        self.model = keras.Model(model_in, model_out, name = "VAE")
    ...
    def _add_bottleneck(self, layer):
        ...
        self.mu = keras.layers.Dense(self.code_size, name="mu")(layer)
        self.log_variance = keras.layers.Dense(self.code_size, name="log_variance")(layer)

        def sample_point_from_gaussian_distribution(args):
            mu, log_var = args
            epsilon = keras.backend.random_normal(shape = keras.backend.shape(self.mu), mean=0., stddev=1.)
            sampled_point = mu + keras.backend.exp(log_var / 2) * epsilon
            return sampled_point

        layer = keras.layers.Lambda(sample_point_from_gaussian_distribution, name="encoder_output")([self.mu, self.log_variance])
...
if __name__ == "__main__":
    ...
    vae.summary()
    vae.compile(lr=LR)
    vae.train(X_train[:10000], batch_size=BATCH_SIZE, epochs=EPOCHS)
    vae.save("vae_model")
    vae.summary()
    ...
    re_im, lat_rep = vae.reconstruct(sample)
