...
class WaveNet(nn.Module):
    def __init__(self, num_channels, dilation_depth, num_repeat, kernel_size=2):
        ...
        dilations = [2 ** d for d in range(dilation_depth)] * num_repeat # wavenet suggests that the dilation patterns should grow exponentially
...
class PedalNet(pl.LightningModule):
    ...
    def configure_optimizers(self):
        return torch.optim.Adam(
            self.wavenet.parameters(), lr=self.hyper_params.learning_rate
        )
    ...
    def val_dataloader(self):
        return DataLoader(
            self.valid_ds, batch_size=self.hyper_params.batch_size, num_workers=4
        )
